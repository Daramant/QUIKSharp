        ...
        async Task GetQuotation(Tool instrument, CandleInterval tf)
        {
            ...
            try
            {
                ...
                while (!_isSubscribed)
                {
                    ...
                    _quik.Functions.Candles.SubscribeAsync(instrument.ClassCode, instrument.SecurityCode, tf).Wait();
                    textBoxLogs.AppendText("Проверяем состояние подписки" + "..." + Environment.NewLine);
                    _isSubscribed = _quik.Functions.Candles.IsSubscribedAsync(instrument.ClassCode, instrument.SecurityCode, tf).Result;
                }
                ...
                AllCandles = await _quik.Functions.Candles.GetAllCandlesAsync(instrument.ClassCode, instrument.SecurityCode, tf).ConfigureAwait(false);
        ...
        void VerifingEntrancePosition()
        {
            if (position.entranceOrderID > 0 || position.entranceOrderNumber > 0)
            {
                if (position.entranceOrderID > 0)
                {
                    ...
                    try
                    {
                        position.entranceOrderNumber = _quik.Functions.TableRows.GetOrderByTransactionIdAsync(tool.ClassCode, tool.SecurityCode, position.entranceOrderID).Result.OrderNum;
                ...
                if (position.entranceOrderNumber > 0)
                {
                    ...
                    try
                    {
                        Order order = _quik.Functions.TableRows.GetOrderByOrderNumberAsync(position.entranceOrderNumber).Result;
                        ...
                        if (order != null && !order.Flags.HasFlag(OrderTradeFlags.Active))
                        {
                            if (order.Balance < order.Quantity)
                            {
                                ...
                                var trades = _quik.Functions.TableRows.GetTradesAsync().Result;
                        ...
                        else
                        {
                            if (settings.LifeTimeOrder != "00:00:00")
                            {
                                ...
                                if (DateTime.Now > orderEndTime)
                                {
                                    ...
                                    _transactionManager.KillOrderAsync(order).Wait();
                                }
                            }
                        }
                    }
        ...
        void VerifingClosingPosition()
        {
            if (position.closingOrderID > 0 || position.closingOrderNumber > 0)
            {
                if (position.closingOrderID > 0)
                {
                    try
                    {
                        position.closingOrderNumber = _quik.Functions.TableRows.GetOrderByTransactionIdAsync(tool.ClassCode, tool.SecurityCode, position.closingOrderID).Result.OrderNum;
                    }
                    catch
                    {
                ...
                if (position.closingOrderNumber > 0)
                {
                    try
                    {
                        Order order = _quik.Functions.TableRows.GetOrderByOrderNumberAsync(position.closingOrderNumber).Result;
                        textBoxOrderDirection.Text = order.Operation.ToString();
                        textBoxOrderBalance.Text = order.Balance.ToString();
                        textBoxOrderNumber.Text = order.OrderNum.ToString();
                        textBoxOrderPrice.Text = order.Price.ToString();
