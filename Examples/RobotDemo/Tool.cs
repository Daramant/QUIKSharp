...
public class Tool   
{
    ...
    public decimal LastPrice
    {
        get
        {
            lastPrice = Convert.ToDecimal(_quik.Functions.Trading.GetParamExAsync(classCode, securityCode, "LAST").Result.ParamValue.Replace('.', separator));
            return lastPrice;
        }
    }
    #endregion
    ...
    void GetBaseParam(IQuik quik, string secCode, string _classCode, int _koefSlip)
    {
        try
        {
            ...
            if (quik != null)
            {
                if (classCode != null && classCode != "")
                {
                    try
                    {
                        name = quik.Functions.Class.GetSecurityInfoAsync(classCode, securityCode).Result.ShortName;
                        accountID = quik.Functions.Class.GetTradeAccountAsync(classCode).Result;
                        firmID = quik.Functions.Class.GetClassInfoAsync(classCode).Result.FirmId;
                        step = Convert.ToDecimal(quik.Functions.Trading.GetParamExAsync(classCode, securityCode, "SEC_PRICE_STEP").Result.ParamValue.Replace('.', separator));
                        slip = _koefSlip * step;
                        priceAccuracy = Convert.ToInt32(Convert.ToDouble(quik.Functions.Trading.GetParamExAsync(classCode, securityCode, "SEC_SCALE").Result.ParamValue.Replace('.', separator)));
                    }
                    catch (Exception e)
                    {
                    ...
                    if (classCode == "SPBFUT")
                    {
                        ...
                        lot = 1;
                        guaranteeProviding = Convert.ToDouble(quik.Functions.Trading.GetParamExAsync(classCode, securityCode, "BUYDEPO").Result.ParamValue.Replace('.', separator));
                    }
                    else
                    {
                        Console.WriteLine("Получаем 'lot'.");
                        lot = Convert.ToInt32(Convert.ToDouble(quik.Functions.Trading.GetParamExAsync(classCode, securityCode, "LOTSIZE").Result.ParamValue.Replace('.', separator)));
                        guaranteeProviding = 0;
                    }
                    try
                    {
                        priceStep = Convert.ToDecimal(quik.Functions.Trading.GetParamExAsync(classCode, securityCode, "STEPPRICET").Result.ParamValue.Replace('.', separator));
                    }
                    catch (Exception e)
                    {
