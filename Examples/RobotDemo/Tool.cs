...
public class Tool   
{
    ...
    public decimal LastPrice
    {
        get
        {
            lastPrice = Convert.ToDecimal(_quik.Functions.Workstation.GetParamExAsync(classCode, securityCode, ParamName.LAST).Result.ParamValue.Replace('.', separator));
            return lastPrice;
        }
    }
    #endregion
        ...
        try
        {
            ...
            if (quik != null)
            {
                if (classCode != null && classCode != "")
                {
                    ...
                    try
                    {
                        priceStep = Convert.ToDecimal(quik.Functions.Workstation.GetParamExAsync(classCode, securityCode, ParamName.STEPPRICET).Result.ParamValue.Replace('.', separator));
                    }
                    catch (Exception e)
                    {
